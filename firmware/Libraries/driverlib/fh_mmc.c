...
int MMC_UpdateClockRegister(struct fh_mmc_obj *mmc_obj, int div)
{
    ...
    tick    = rt_tick_get();
    timeout = tick + MMC_UPDATE_CLK_TIMEOUT;  // 1s in total
                    ...
                    rt_uint32_t arg, rt_uint32_t flags)
{
    ...
    tick    = rt_tick_get();
    timeout = tick + MMC_SEND_CMD_TIMEOUT;  // 1s
...
int MMC_ResetFifo(struct fh_mmc_obj *mmc_obj)
{
    ...
    tick    = rt_tick_get();
    timeout = tick + MMC_RESET_FIFO_TIMEOUT;  // 1s
...
int MMC_Reset(struct fh_mmc_obj *mmc_obj)
{
    ...
    tick    = rt_tick_get();
    timeout = tick + MMC_RESET_SDC_TIMEOUT;  // 1s
...
void MMC_Init(struct fh_mmc_obj *mmc_obj)
{
    ...
    if (mmc_obj->mmc_reset) mmc_obj->mmc_reset(mmc_obj);
