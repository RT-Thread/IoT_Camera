...
rt_uint32_t gpio_to_irq(rt_uint32_t gpio) { return (gpio + NR_INTERNAL_IRQS); }
...
int gpio_request(rt_uint32_t gpio)
{
    if (gpio > NUM_OF_GPIO)
    {
        rt_kprintf("ERROR: %s, incorrect GPIO num: %d\n", __func__, gpio);
        return -RT_ERROR;
    }

    if(gpio_available[gpio] == 1)
    {
        rt_kprintf("ERROR: %s, GPIO %d is already in use\n", __func__, gpio);
        return -RT_EBUSY;
    }
