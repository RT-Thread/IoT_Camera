...
static char *find_pair(char *, const char);	// find matching pair ()  []  {}
#if !ENABLE_FEATURE_VI_UNDO
...
#else
static int query_screen_dimensions(void) { return 0; }
#endif
...
static int find_range(char **start, char **stop, char c)
{
	...
	if (strchr("cdy><", c)) {
		...
		if (dot > text && ((isspace((unsigned char)dot[-1]) && !isspace((unsigned char)dot[0]))
				|| (ispunct((unsigned char)dot[-1]) && !ispunct((unsigned char)dot[0]))
				|| (isalnum((unsigned char)dot[-1]) && !isalnum((unsigned char)dot[0]))))
...
static int st_test(char *p, int type, int dir, char *tested)
{
	...
	if (type == S_BEFORE_WS) {
		c = ci;
		test = (!isspace((unsigned char)c) || c == '\n');
	}
	if (type == S_TO_WS) {
		c = c0;
		test = (!isspace((unsigned char)c) || c == '\n');
	...
	if (type == S_OVER_WS) {
		c = c0;
		test = isspace((unsigned char)c);
	}
	if (type == S_END_PUNCT) {
		c = ci;
		test = ispunct((unsigned char)c);
	}
	if (type == S_END_ALNUM) {
		c = ci;
		test = (isalnum((unsigned char)c) || c == '_');
	}
	*tested = c;
	return test;
}
		...
		do {
			...
			if (dot < end - 1) {	// make sure not last char in text[]
				...
				*dot++ = ' ';
				modified_count++;
#endif
				while (isblank((unsigned char)*dot)) {	// delete leading WS
	...
	case '~':			// ~- flip the case of letters   a-z -> A-Z
		do {
			...
			if (islower((unsigned char)*dot)) {
				*dot = toupper((unsigned char)*dot);
				modified_count++;
			} else if (isupper((unsigned char)*dot)) {
				*dot = tolower((unsigned char)*dot);
				modified_count++;
			}
#endif
			dot_right();
...
FINSH_FUNCTION_EXPORT(vi, vi in ms);
FINSH_FUNCTION_EXPORT_ALIAS(vi_main, __cmd_vi, vi in ms);
